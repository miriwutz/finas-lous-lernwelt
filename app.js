...
$("#loginForm").addEventListener("submit", async e => {
  ...
  try{
    ...
    $("#loginMessage").textContent = "Anmeldung nicht möglich. Bitte E-Mail und Passwort prüfen.";
  }
});
...
function renderAll(){
  const now = new Date();
  $("#todayLabel").textContent = now.toLocaleDateString("de-AT",{weekday:"long",day:"2-digit",month:"long",year:"numeric"});
...
function renderTasks(child){
  ...
  tasks.forEach(task=>{
    ...
    row.className = "task";
    row.innerHTML = `
      <button class="leaf-toggle ${task.done?"done":""}" aria-label="Aufgabe ${task.done?"wieder öffnen":"abschließen"}">${leafSvg(task.done)}</button>
      <div class="task-main">
        <div class="task-title">${escapeHtml(task.title)}</div>
        <div class="task-note">${escapeHtml(task.note||"")}</div>
      </div>
      ${task.type==="online" && task.url ? `<a class="task-link" href="${escapeHtml(task.url)}" target="_blank" rel="noopener">Öffnen ↗</a>` : ""}
    `;
  ...
  try{
    await runTransaction(db, async tx => {
      ...
      if(task.done && leafIndex<0){
        leaves.push({id:crypto.randomUUID(),taskId,child:task.child,title:task.title,createdAt:new Date().toISOString()});
      }
      if(!task.done && leafIndex>=0) leaves.splice(leafIndex,1);

      tx.update(spaceRef,{tasks,learningLeaves:leaves,updatedAt:serverTimestamp()});
    });
  }catch(err){ alert("Die Aufgabe konnte nicht gespeichert werden: "+err.message); }
}
...
function renderTree(){
  ...
  leafLayer.innerHTML = flowerLayer.innerHTML = rootLayer.innerHTML = "";
  ...
  leaves.forEach((leaf,i)=>{
    ...
    leafLayer.insertAdjacentHTML("beforeend",`
      <g transform="translate(${x} ${y}) rotate(${rot}) scale(1 ${mirror})" class="dynamic-leaf">
        <path d="M0 0 C8 -16 25 -18 35 -4 C25 10 10 12 0 0Z" fill="${color}" stroke="#765f52" stroke-width="1.2"/>
        <path d="M3 0 C14 -1 24 -2 32 -4" fill="none" stroke="#fffaf4" stroke-width="1.15"/>
      </g>`);
  });
  ...
  (state.roots||[]).slice(0,rootPaths.length).forEach((root,i)=>{
    rootLayer.insertAdjacentHTML("beforeend",`<path d="${rootPaths[i]}" class="dynamic-root" stroke-width="${Math.max(4,8-i*.25)}"/>`);
  });
...
function renderHearts(){
  ...
  heartOptions.forEach(opt=>{
    ...
    btn.className="heart-choice"+(selectedHearts.includes(opt.key)?" selected":"");
    btn.innerHTML=`<span class="heart-icon">${opt.icon}</span><strong>${opt.title}</strong><span>${opt.text}</span>`;
    btn.onclick=()=>{
      selectedHearts = selectedHearts.includes(opt.key)
  ...
  try{
    await runTransaction(db, async tx=>{
      ...
      roots.push({
        ...
        author:$("#heartAuthor").value,
        reason,
        createdAt:new Date().toISOString()
      });
      tx.update(spaceRef,{roots,updatedAt:serverTimestamp()});
    });
    selectedHearts=[];
    $("#heartReason").value="";
    renderHearts();
  ...
  try{
    await runTransaction(db, async tx=>{
      ...
      tx.update(spaceRef,{
        ...
        roots:[],
        tasks,
        tree:{...data.tree,name:"Neuer Wochenbaum",startedAt:new Date().toISOString()},
        updatedAt:serverTimestamp()
      });
    });
  }catch(err){ alert("Der Baum konnte nicht in den Lernwald gesetzt werden: "+err.message); }
};
  ...
  forest.forEach(tree=>{
    box.insertAdjacentHTML("beforeend",`
      <article class="forest-tree">
        ...
        <div class="muted">${new Date(tree.completedAt).toLocaleDateString("de-AT",{month:"long",year:"numeric"})}</div>
...
$("#unlockMamaBtn").onclick=async()=>{
  ...
  try{
    ...
    $("#mamaUnlockMessage").textContent="Das Passwort stimmt nicht.";
  }
};
...
function prepareAdmin(){
  ...
  $("#treeNameInput").value=state.tree?.name||"";
...
function renderAdminTasks(child){
  ...
  adminDraft[child].forEach((task,index)=>{
    ...
    row.innerHTML=`
      ...
      <input class="full" data-field="note" value="${escapeHtml(task.note||"")}" placeholder="Kurze Anweisung">
      <input class="full" data-field="url" value="${escapeHtml(task.url||"")}" placeholder="Link – nur bei Lernhomepage">`;
    row.querySelectorAll("[data-field]").forEach(el=>{
      el.oninput=()=>adminDraft[child][index][el.dataset.field]=el.value;
    });
...
$$("[data-add-task]").forEach(btn=>btn.onclick=()=>{
  const child=btn.dataset.addTask;
  adminDraft[child].push({id:crypto.randomUUID(),child,title:"",note:"",type:"paper",url:"",done:false});
  renderAdminTasks(child);
});

$("#saveTasksBtn").onclick=async()=>{
  const tasks=[...adminDraft.fina,...adminDraft.lou].filter(t=>t.title.trim());
  try{
    await updateDoc(spaceRef,{tasks,updatedAt:serverTimestamp()});
    alert("Die Aufgaben sind jetzt auf allen Geräten veröffentlicht.");
  }catch(err){alert("Speichern nicht möglich: "+err.message);}
};
